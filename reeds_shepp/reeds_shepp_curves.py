...
def create_left_arc(center, radius, limit, interval=5):
    n = abs(int((limit[1]-limit[0])/interval))
    angles = list(np.linspace(*limit, n+1, endpoint=True))
    return [[center[0] + radius * np.cos(np.deg2rad(a)-np.pi/2),
             center[1] + radius * np.sin(np.deg2rad(a)-np.pi/2), a]
            for a in angles]


def create_right_arc(center, radius, limit, interval=5):
    n = abs(int((limit[1]-limit[0])/interval))
    angles = list(np.linspace(*limit, n+1, endpoint=True))
    return [[center[0] + radius * np.cos(np.deg2rad(a)+np.pi/2),
             center[1] + radius * np.sin(np.deg2rad(a)+np.pi/2), a]
...
class Action(object):
    ...
    def __init__(self, start=None, goal=None, radius=0,
                 is_rotate=False, is_left=True, is_forward=True):
    ...
    def rotate(self, yaw, radius, is_left=True, is_forward=True):
        ...
        return Action(q0, q1, radius, True, is_left, is_forward)

    @staticmethod
    def scs(t, u, v):
        actions = [Action().move(t)]
        actions.append(actions[-1].rotate(*u))
        actions.append(actions[-1].move(v))
        points = []
        for a in actions:
    ...
    @staticmethod
    def csc(t, u, v):
        actions = [Action().rotate(*t)]
        actions.append(actions[-1].move(u))
        actions.append(actions[-1].rotate(*v))
        points = []
        for a in actions:
            points.extend(a.to_points())
        return points
    ...
    def to_points(self):
        ...
        if self.is_left and self.is_forward:
            if limit[1] < limit[0]:
                limit[1] += 360
            return create_left_arc(center, self.radius, limit)
            ...
            if limit[1] < limit[0]:
                limit[1] += 360
            return create_right_arc(center, self.radius, limit)
        else:
            if limit[1] > limit[0]:
                limit[1] -= 360
            return create_right_arc(center, self.radius, limit)
...
class Parameters(object):
    ...
    def to_local(self, q):
        ...
        return [xy[0, 0], xy[1, 0], q[2]-self.q0[2]]
    ...
    def scs(self):
        ...
        if (self.loc_q1[2] > 0) != (self.loc_q1[1] > 0):
            v = -v
        paths.append(Action.scs(t, (u, radius, True, True), v))
        paths.append(Action.scs(t, (theta, radius, True, False), v))

        # turning right
        t, u, v = xd + xd_arc, theta, yd+yd_arc
        if (self.loc_q1[2] > 0) != (self.loc_q1[1] > 0):
            v = -v
        paths.append(Action.scs(t, (u, radius, False, True), v))
        paths.append(Action.scs(t, (u, radius, False, False), v))
    ...
    def lsl(self, radius, t, u, v):
        paths = []
        for i in range(1 << 2):
            paths.append(Action.csc((t, radius, True, not (i & 0b1)),
                                    u, (v, radius, True, not (i & 0b10))))
        return paths

    def rsr(self, radius, t, u, v):
        paths = []
        for i in range(1 << 2):
            paths.append(Action.csc((t, radius, False, not (i & 0b1)),
                                    u, (v, radius, False, not (i & 0b10))))
        return paths
    ...
    def lrl(self, radius, t, u, v):
        paths = []
        for i in range(1 << 3):
            paths.append(Action.ccc((t, radius, True, not (i & 0b1)),
    ...
    def rlr(self, radius, t, u, v):
        paths = []
        for i in range(1 << 3):
            paths.append(Action.ccc((t, radius, False, not (i & 0b1)),
...
def plot_paths(params, paths):
    ...
    for path in paths:
        ...
        length = sum([np.linalg.norm(np.subtract(
            points[i+1][0:2], points[i][0:2])) for i in range(len(points)-1)])
    ...
    plt.arrow(params.q0[0], params.q0[1], 10*np.cos(np.deg2rad(params.q0[2])),
              10*np.sin(np.deg2rad(params.q0[2])), head_width=5)

    plt.arrow(params.q1[0], params.q1[1], 10*np.cos(np.deg2rad(params.q1[2])),
              10*np.sin(np.deg2rad(params.q1[2])), head_width=5)
    plt.axis('equal')
    # plt.grid(True)
